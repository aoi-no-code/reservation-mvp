...
export async function GET(
  ...
  if (!token?.trim()) {
    return NextResponse.redirect(new URL('/today?error=invalid', process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'));
  }
  ...
  if (confirmError || !reservationId) {
    ...
    return NextResponse.redirect(
      new URL('/today?error=confirm_failed', process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000')
    );
  }
