...
import { TodaySlots } from './today/TodaySlots';
...
function getHourlyRateSummary(slots: { stylist_hourly_rate: number | null }[]): string | null {
  const rates = Array.from(
    new Set(slots.map((s) => s.stylist_hourly_rate).filter((r): r is number => r != null))
  );
  ...
  return (
    ...
    >
      <main className="w-full max-w-[430px] mx-auto px-0 py-10 pb-28 flex flex-col items-center text-center">
        <h1 className="text-2xl font-semibold tracking-tight text-stone-900 mb-2">直近のご案内可能枠</h1>
