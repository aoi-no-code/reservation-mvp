...
type Props = { params: Promise<{ stylistId: string }> };

export async function generateMetadata({ params }: Props) {
  const { stylistId } = await params;
  const name = await getStylistPublicName(stylistId);
  if (!name) return { title: '予約' };
  return {
    title: `${name}さんのご案内可能枠 | 予約`,
    description: `${name}さんの本日ご案内可能な限定枠からご予約いただけます。`,
    openGraph: {
      title: `${name}さんのご案内可能枠`,
      description: '限定枠からご予約をどうぞ。',
    },
  };
}

export default async function StylistReservationPage({ params }: Props) {
  const { stylistId } = await params;
  const [stylistName, slots] = await Promise.all([
    getStylistPublicName(stylistId),
    getPublicSlotsByStylist(stylistId),
  ]);
